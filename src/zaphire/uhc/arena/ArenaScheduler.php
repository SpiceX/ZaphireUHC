        ...
        switch ($this->arena->phase) {
            case Arena::PHASE_GAME:
                ...
                $this->arena->updateScoreboard([
                    "§7------------------ ",
                    "§cRemaining: §7" . count($this->arena->players),
                    "§cKills: §7{KILLS}",
                    "§7------------------",
                    "§cBorder: §7" . $this->arena->border,
                    "§cCenter: §7{DISTANCE}",
                    "§cScenarios:",
                ]);
                $this->arena->updateBossbar("§c§lMap: §r§7 " . $this->arena->level->getFolderName() . " §l§6» §r§7" . Time::calculateTime($this->gameTime));
                switch ($this->gameTime) {
                    case 55 * 60:
                        $this->arena->enablePvP();
                        $this->arena->broadcastMessage("§c§l» §r§7PvP has been enabled!");
                        $this->arena->broadcastMessage("§c§l» §r§7Border will shrink 100 blocks in 10 minutes.");
                        break;
                    case 50 * 60:
                        $this->arena->broadcastMessage("§c§l» §r§7Border will shrink 100 blocks in 5 minutes.");
                        break;
                    case 46 * 60:
                        $this->arena->broadcastMessage("§c§l» §r§7Border will shrink 100 blocks in 1 minute.");
                        break;
                    case 45 * 60:
                        $this->arena->shrinkEdge(100);
                        $this->arena->broadcastMessage("§c§l» §r§7Border is now 900 blocks.");
                        break;
                    case 35 * 60:
                        $this->arena->broadcastMessage("§c§l» §r§7Continous shrink starts in 5 minutes.");
                        break;
                    case 30 * 60:
                        $this->arena->broadcastMessage("§c§l» §r§7Continous shrink has been started.");
                        break;
                }
            ...
            case Arena::PHASE_RESTART:
                $this->arena->broadcastMessage("§c§l» §r§7Restarting in {$this->restartTime} sec.", Arena::MSG_TIP);
                $this->restartTime--;
                if (count($this->arena->players) === 1) {
                    foreach ($this->arena->players as $player) {
                        $this->arena->addFireworks($player);
                    }
                }
                switch ($this->restartTime) {
                    case 0:
                        ...
                        foreach ($this->arena->spectators as $player) {
                            $player->teleport($this->arena->plugin->getServer()->getDefaultLevel()->getSpawnLocation());
                            $player->getInventory()->clearAll();
                            $player->getArmorInventory()->clearAll();
                            $player->getCursorInventory()->clearAll();
