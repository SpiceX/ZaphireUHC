...
use Exception;
use zaphire\uhc\arena\scenario\LimitationsStorage;
use zaphire\uhc\arena\scenario\Scenarios;
use zaphire\uhc\arena\scenario\TimeBombTask;
use zaphire\uhc\arena\utils\KillsManager;
use zaphire\uhc\arena\utils\MapReset;
use zaphire\uhc\arena\utils\VoteManager;
...
class Arena extends Game implements Listener
{
     ...
     */
    public function __construct(ZaphireUHC $plugin, array $arenaFileData)
    {
        $this->plugin = $plugin;
    ...
    public function joinToArena(Player $player)
    {
        ...
        $player->setScoreTag("§c❤§f" . round($player->getHealth()));
    ...
    public function onPlayerDeath(PlayerDeathEvent $event)
    {
        ...
        if ($deathMessage === null) {
            $this->broadcastMessage("§c§l» §r§7 {$player->getName()} died. §c(" . count($this->players) . "/{$this->data["slots"]})");
        } else {
            if ($deathMessage instanceof TextContainer) {
                $this->broadcastMessage("§c§l» §r§7 {$this->plugin->getServer()->getLanguage()->translate($deathMessage)} §c(" . count($this->players) . "/{$this->data["slots"]})");
            }
        }
