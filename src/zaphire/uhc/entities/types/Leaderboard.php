...
class Leaderboard extends Human
{
    ...
    public function entityBaseTick(int $tickDiff = 1): bool
    {
        ...
        $y = $this->getY();
        $z = $this->getZ();
        $this->level->addParticle(new GenericParticle(new Vector3($x - $b, $y + $c + $d + 1.2, $z - $a), Particle::TYPE_DRAGONS_BREATH));
        $this->level->addParticle(new GenericParticle(new Vector3($x + $a, $y + $c + $d + 1.2, $z + $b), Particle::TYPE_DRAGONS_BREATH));
        $this->level->addParticle(new GenericParticle(new Vector3($x + $b, $y + $c + $d + 1.2, $z - $a), Particle::TYPE_DRAGONS_BREATH));
        $this->level->addParticle(new GenericParticle(new Vector3($x + $a, $y + $c + $d + 1.2, $z - $b), Particle::TYPE_DRAGONS_BREATH));
    ...
    {
        return ZaphireUHC::getInstance()->getSqliteProvider()->getGlobalTops();
    }
}